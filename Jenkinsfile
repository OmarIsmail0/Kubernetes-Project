                ...
                dir('frontend') {
                    script {
                        ...
                        docker.withRegistry('', DOCKERHUB_CREDENTIALS_ID) {
                            ocker.image("${DOCKERHUB_USERNAME}/${DOCKERHUB_REPOSITORY}:${FRONTEND_TAG}").push("${FRONTEND_TAG}")
                        }
                    }
                }
            }
            ...
            steps {
                dir('backend') {
                    script {
                        ...
                        docker.withRegistry('', DOCKERHUB_CREDENTIALS_ID) {
                            ocker.image("${DOCKERHUB_USERNAME}/${DOCKERHUB_REPOSITORY}:${BACKEND_TAG}").push("${BACKEND_TAG}")
                        }
                    }
                }
            }
