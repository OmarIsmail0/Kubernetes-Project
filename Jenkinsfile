    ...
    stages {
        ...
        stage('Build Frontend Image') {
            steps {
                dir('frontend'){
                    script {
                        docker.build("${DOCKERHUB_USERNAME}/${DOCKERHUB_REPOSITORY}:${FRONTEND_TAG}", "-f ./frontend/Dockerfile .")
        ...
        stage('Build Backend Image') {
            steps {
                dir('backend') {
                    script {
                        docker.build("${DOCKERHUB_USERNAME}/${DOCKERHUB_REPOSITORY}:${BACKEND_TAG}", "-f ./backend/Dockerfile .")
                        docker.withRegistry('', DOCKERHUB_CREDENTIALS_ID) {
